 ...
 * different model, and flip the whole app between Anthropic and Groq with a
 * one-line config change — no code edit.
 *
 * Resolution order per role:
 *   1. per-role env override  OZ_MODEL_<ROLE>  (format "provider:modelId")
 *   2. MODEL_PROFILE env       ("anthropic" | "groq" | "mixed")
 *   3. default                 "anthropic"
 *
 * SECURITY:
 *   - Server-only module. Keys (ANTHROPIC_API_KEY / GROQ_API_KEY) are read from
 *     process.env here only — NEVER NEXT_PUBLIC_.
...
type ProfileName = "anthropic" | "groq" | "mixed";
...
function resolveProfile(): ProfileName {
  ...
  if (raw === "groq" || raw === "mixed" || raw === "anthropic") {
    return raw;
  }
